...
* It does not make any making assumptions to fill in missing values.
* It corrects only *one* values which appear to be incorrect. As the section [District-Level Main Source of Information Data] illustrates, the total number of households in Western Area Urban in the sources of information data is missing a final "1", and should be "229951" instead of "22995".
...
For this notebook, we use R and the [tidyverse](https://www.tidyverse.org/) collection of packages to produce datasets in "tidy" format for immediate use in data visualisation and analysis.
...
**4. Main Source of Information at the District Level:** Data from the 2015 Population and Housing Census (PHC), sourced from [Open Data Sierra Leone](https://opendatasl.gov.sl/dataset/households-main-source-information-district). The dataset presents the main sources of information, such as television or radio, for households in the 14 districts of Sierra Leone. Last accessed 2 June 2020.
...
The same mapping data may instead be accessed in GeoJSON format using the ArcGIS Hub [APIs](https://hub.arcgis.com/datasets/DirectRelief::sierre-leone-districts) on the dataset's web page. I use the GeoJSON data to [interactively map coronavirus by district using D3.js](https://observablehq.com/@todowa/coronavirus-covid-19-in-sierra-leone).
...
We graph the number of cases by district:
...
We quickly perform some checks to ensure that the dataset makes sense:
...
We expand the time series to include all dates between the first and last recorded date, even those on which no data are reported:
...
* `decrease`: Dummy variable indicating whether the number of reported cases has been revised down. NA if no reported cases on that date; 1 if there is a decrease from the last reported cases; 0 otherwise
...
We confirm that the 14 `district`, `d_code` and `d_id` match those in the demographics dataset.
...
We check that the total number of households matches the sum across the different sources of information:
...
We export the tibble as a csv:
```{r}
write_csv(info_district, file.path(OUTPUT_PATH, CV_NATIONAL_OUT))
```
